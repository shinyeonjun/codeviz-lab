...
RUNNER_SCRIPT_PATH = Path(__file__).with_name("java_execute_runner.py")


class LocalJavaExecutionRunner:
    def __init__(
        self,
        *,
        timeout_seconds: int,
    ...
    def run(self, command: TraceExecutionCommand) -> TraceExecutionResult:
        ...
        if completed.returncode != 0:
            message = stderr_text.strip() or "Java 실행 결과를 생성하지 못했습니다."
        ...
        return parse_trace_result_payload(
            ...
            invalid_payload_message="Java 실행 결과를 해석하지 못했습니다.",
        )
