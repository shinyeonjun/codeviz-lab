...
class DockerJavaExecutionRunner:
    def __init__(
        self,
        *,
        timeout_seconds: int,
    ...
    def run(self, command: TraceExecutionCommand) -> TraceExecutionResult:
        if command.language != "java":
            return TraceExecutionResult.unsupported_language(language=command.language)

        container_name = f"codeviz-java-runner-{uuid.uuid4().hex[:12]}"
        ...
        if completed.returncode != 0:
            message = stderr_text.strip() or "Docker Java 실행에 실패했습니다."
            return TraceExecutionResult.failure(
                language=command.language,
                message=message,
            )
        ...
        return parse_trace_result_payload(
            ...
            invalid_payload_message="Docker Java 실행 결과를 해석하지 못했습니다.",
        )
    ...
    def _build_command(self, *, container_name: str) -> list[str]:
        return [
            ...
            self._cpus,
            "--tmpfs",
            f"/tmp:rw,exec,nosuid,size={self._tmpfs_size}",
            "--init",
            "-e",
