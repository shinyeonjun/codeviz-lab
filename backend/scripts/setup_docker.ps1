...
$javaSandboxImage = "codeviz-java-sandbox:latest"
$composeFile = Join-Path $projectRoot "docker-compose.yml"

Write-Host "[1/5] Docker 상태 확인 중..."
...
Write-Host "[4/5] Java sandbox 이미지 빌드 중..."
docker build -t $javaSandboxImage -f "$projectRoot\docker\java-runner\Dockerfile" $projectRoot

Write-Host "[5/5] PostgreSQL 컨테이너 시작 중..."
...
Write-Host "PostgreSQL health check 대기 중..."
